...
const createPromise = <T>(createItemFn: () => T) =>
  new Promise<T>((resolve, reject) => resolve(createItemFn()));
...
const mockItemTaskManager: ItemTaskManager = {
  ...
  createCreateTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItem)),
  createGetTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItem)),
  createUpdateTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItem)),
  createDeleteTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItem)),
  createMoveTask: jest
    .fn()
  ...
  createCopyTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItem)),
  createGetChildrenTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItemArray)),
  createGetOwnTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItemArray)),
  createGetSharedWithTask: jest
    .fn()
    .mockReturnValue(createMockTask(createMockActor, createMockItemArray)),
};
...
const mockItemMembershipService: ItemMembershipService = {
  ...
  getInherited: jest
    .fn()
    .mockReturnValue(createPromise(createMockItemMembership)),
  getAllBelow: jest
    .fn()
    .mockReturnValue(createPromise(createMockItemMembershipArray)),
  getInheritedForAll: jest
    .fn()
    .mockReturnValue(createPromise(createMockItemMembershipArray)),
  ...
  deleteManyMatching: jest
    .fn()
    .mockReturnValue(createPromise(createMockItemMembershipArray)),
  moveHousekeeping: jest.fn().mockReturnValue(
    createPromise(() => ({
...
export const mockTaskRunner: TaskRunner<Actor> = {
  ...
  runMultiple: (tasks: Task<Actor, unknown>[]) =>
    createPromise(() => tasks.map((t) => t.result)),
  setTaskPreHookHandler: (taskName, handler) => {
    const preHandlers = mockTaskRunnerState.handlers.pre;
    preHandlers.has(taskName)
      ? preHandlers.get(taskName)?.push(handler)
      : preHandlers.set(taskName, [handler]);
  },
  setTaskPostHookHandler: (taskName, handler) => {
    const postHandlers = mockTaskRunnerState.handlers.post;
    postHandlers.has(taskName)
      ? postHandlers.get(taskName)?.push(handler)
      : postHandlers.set(taskName, [handler]);
  },
  unsetTaskPreHookHandler: (taskName, handler) => {
    ...
    if (taskPreHandlers !== undefined) {
      preHandlers.set(
        taskName,
        taskPreHandlers.filter((h) => h !== handler),
      );
    }
  },
  unsetTaskPostHookHandler: (taskName, handler) => {
    ...
    if (taskPostHandlers !== undefined) {
      postHandlers.set(
        taskName,
        taskPostHandlers.filter((h) => h !== handler),
      );
    }
  },
  runPre: <T>(taskName: string, param: T) =>
    Promise.all(
      mockTaskRunnerState.handlers.pre
        .get(taskName)
        ?.map(async (h) => await h(param, createMockActor(), mockHelpers)) ??
        [],
    ),
  runPost: <T>(taskName: string, param: T) =>
    Promise.all(
      mockTaskRunnerState.handlers.post
        .get(taskName)
        ?.map(async (h) => await h(param, createMockActor(), mockHelpers)) ??
        [],
    ),
  clearHandlers: () =>
    ['pre', 'post'].forEach((p) => mockTaskRunnerState.handlers[p].clear()),
};

...
function runClients() {
    const connString = `ws://${ADDRESS}:${PORT}`;
    const sockets = Array(N_CLIENTS).fill(null).map(_ => new WebSocket(connString));
    ...
    const receiver = new WebSocket(connString);
    receiver.on('message', data => console.log(`Client received: ${data}`));

    const sender = new WebSocket(connString);
